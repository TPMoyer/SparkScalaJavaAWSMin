...
object FileSystemManager {
	 ...
	 * Uses the typesafe's config
	 * 
	 * On cluster: defaults to hdfs 
	 * useLocalFs: optional parameter which allows direction to the 
	 * potentially different file system local to the spark-submit ie: the edge node local file system.
	 */
