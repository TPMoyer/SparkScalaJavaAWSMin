...
object MiscellaneousUtils  {
	...
	def  showHiveDatabases(spark : SparkSession):String={
		val sb = new StringBuilder()
		sb.append("cme@ showHiveDatabases\n")
		val df=spark.sql("show databases")
		if(1==df.count){
			sb.append("if this shows only default, the likely situation is that you are pointing to a self generated warehouse in   System.getProperty(\"user.dir\")/spark-warehouse\n")
			sb.append("on HDP systems check Ambari... hosts... hive... config.... and scroll down to  hive.metastore.warehouse.dir\n")
			sb.append("As of spark2.X this location needs to be set into spark.sql.warehouse.dir=hdfs///apps/hive/warehouse\n")    
			sb.append("this is most easily done as a -Doption=value   for example    -Dspark.sql.warehouse.dir=hdfs///apps/hive/warehouse\n")
			}
		sb.append("show databases row count="+df.count+"\n"+showString(df))
		sb.toString
	}
	def printAndLog(msg:String) {println(msg);log.info(msg+"\n")}
	def logAndPrint(msg:String) {println(msg);log.info(msg+"\n")}
	def logNPrint  (msg:String) {println(msg);log.info(msg+"\n")}
	def lnp        (msg:String) {println(msg);log.info(msg+"\n")}
...
//    if(1==rddNumPartitions)return(1)
		...
		*/
		FileSystemManager.fileSystem(config).delete(new Path(fidOut+"_test"),true)
...
//    }
		...
		var loff=getArrayOfFileStatuses(config,fidOut+"_test" ,true).filter(f => f.getPath.toString.endsWith(".gz.parquet"))
	...
	def prettyReNameMROutput(
		...
		var loff=getArrayOfFileStatuses(config,path,false)
		...
		*  */
		...
		FileSystemManager.fileSystem(config).rename(f.getPath,new Path(newName))
		})
	}
	...
	*/
	...
	if (hasMoreData) {
		...
		val nonReservedKeywords=List("ADD","ADMIN","AFTER","ANALYZE","ARCHIVE","ASC","BEFORE","BUCKET","BUCKETS","CASCADE","CHANGE","CLUSTER","CLUSTERED","CLUSTERSTATUS","COLLECTION"
				,"COLUMNS","COMMENT","COMPACT","COMPACTIONS","COMPUTE","CONCATENATE","CONTINUE","DATA","DATABASES","DATETIME","DAY","DBPROPERTIES","DEFERRED","DEFINED","DELIMITED"
				,"DEPENDENCY","DESC","DIRECTORIES","DIRECTORY","DISABLE","DISTRIBUTE","ELEM_TYPE","ENABLE","ESCAPED","EXCLUSIVE","EXPLAIN","EXPORT","FIELDS","FILE","FILEFORMAT","FIRST"
				,"FORMAT","FORMATTED","FUNCTIONS","HOLD_DDLTIME","HOUR","IDXPROPERTIES","IGNORE","INDEX","INDEXES","INPATH","INPUTDRIVER","INPUTFORMAT","ITEMS","JAR","KEYS","KEY_TYPE"
				,"LIMIT","LINES","LOAD","LOCATION","LOCK","LOCKS","LOGICAL","LONG","MAPJOIN","MATERIALIZED","MINUS","MINUTE","MONTH","MSCK","NOSCAN","NO_DROP","OFFLINE","OPTION"
				,"OUTPUTDRIVER","OUTPUTFORMAT","OVERWRITE","OWNER","PARTITIONED","PARTITIONS","PLUS","PRETTY","PRINCIPALS","PROTECTION","PURGE","READ","READONLY","REBUILD","RECORDREADER"
				,"RECORDWRITER","REGEXP","RELOAD","RENAME","REPAIR","REPLACE","RESTRICT","REWRITE","RLIKE","ROLE","ROLES","SCHEMA","SCHEMAS","SECOND","SEMI","SERDE","SERDEPROPERTIES"
				,"SERVER","SETS","SHARED","SHOW","SHOW_DATABASE","SKEWED","SORT","SORTED","SSL","STATISTICS","STORED","STREAMTABLE","STRING","STRUCT","TABLES","TBLPROPERTIES","TEMPORARY"
				,"TERMINATED","TINYINT","TOUCH","TRANSACTIONS","UNARCHIVE","UNDO","UNIONTYPE","UNLOCK","UNSET","UNSIGNED","URI","USE","UTC","UTCTIMESTAMP","VALUE_TYPE","VIEW","WHILE","YEAR")
		// Reserved Words
		val reservedWords=List("ALL","ALTER","AND","ARRAY","AS","AUTHORIZATION","BETWEEN","BIGINT","BINARY","BOOLEAN","BOTH","BY","CASE","CAST","CHAR","COLUMN","CONF","CREATE","CROSS"
				,"CUBE","CURRENT","CURRENT_DATE","CURRENT_TIMESTAMP","CURSOR","DATABASE","DATE","DECIMAL","DELETE","DESCRIBE","DISTINCT","DOUBLE","DROP","ELSE","END","EXCHANGE","EXISTS"
				,"EXTENDED","EXTERNAL","FALSE","FETCH","FLOAT","FOLLOWING","FOR","FROM","FULL","FUNCTION","GRANT","GROUP","GROUPING","HAVING","IF","IMPORT","IN","INNER","INSERT","INT"
				,"INTERSECT","INTERVAL","INTO","IS","JOIN","LATERAL","LEFT","LESS","LIKE","LOCAL","MACRO","MAP","MORE","NONE","NOT","NULL","OF","ON","OR","ORDER","OUT","OUTER","OVER"
				,"PARTIALSCAN","PARTITION","PERCENT","PRECEDING","PRESERVE","PROCEDURE","RANGE","READS","REDUCE","REGEXP","REVOKE","RIGHT","RLIKE","ROLLUP","ROW","ROWS","SELECT","SET"
				,"SMALLINT","TABLE","TABLESAMPLE","THEN","TIMESTAMP","TO","TRANSFORM","TRIGGER","TRUE","TRUNCATE","UNBOUNDED","UNION","UNIQUEJOIN","UPDATE","USER","USING","VALUES","VARCHAR"
				,"WHEN","WHERE","WINDOW","WITH")
		
		nonReservedKeywords++reservedWords
	}
	def getHiveReservedWords(): List[String] = {log.info("cme@ getHiveForbiddenWords")
		// Reserved Words
		val reservedWords=List("ALL","ALTER","AND","ARRAY","AS","AUTHORIZATION","BETWEEN","BIGINT","BINARY","BOOLEAN","BOTH","BY","CASE","CAST","CHAR","COLUMN","CONF","CREATE","CROSS"
				,"CUBE","CURRENT","CURRENT_DATE","CURRENT_TIMESTAMP","CURSOR","DATABASE","DATE","DECIMAL","DELETE","DESCRIBE","DISTINCT","DOUBLE","DROP","ELSE","END","EXCHANGE","EXISTS"
				,"EXTENDED","EXTERNAL","FALSE","FETCH","FLOAT","FOLLOWING","FOR","FROM","FULL","FUNCTION","GRANT","GROUP","GROUPING","HAVING","IF","IMPORT","IN","INNER","INSERT","INT"
