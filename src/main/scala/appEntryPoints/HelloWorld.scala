...
object HelloWorld extends ProcessBase {
	...
	/* yes-spark version of run is present to allow trait (ProcessBase.scala) to be created. It needs an overload version of run called by go4it with includeSpark==true*/
	override def run(config: Config, spark: SparkSession) : Integer = {0} 
	...
	override def run(config: Config) : Integer = {
		...
		printAndLog("\nHello World:   config.onCluster="+config.getBoolean("onCluster"))
