...
object coalesceParquets {
  ...
  def yourStuffGoesHere(
    ...
    println("Hello World")
    val fidIn=config.getString("directoryWithTooManyParquetFiles")
    val fidOut=config.getString("outputDirectory")
    val aofs=getArrayOfFileStatuses(fs,fidIn,false)
    var cumSize=0L
    for((f,ii)<-aofs.zipWithIndex){
      cumSize+=f.getLen
    }
    log.info("cumSize="+cumSize)
    val historicFudgeFactor =1.2
    var desiredNumPartitions=(historicFudgeFactor*cumSize/config.getLong("outputParquetFileSizeDesired")).toInt
    log.info("initial take on desiredNumPartitions="+desiredNumPartitions)
    if(1>desiredNumPartitions)desiredNumPartitions=1 
    
    log.info("about to attempt read on "+fidIn)
    val df = spark.read.parquet(fidIn)
    for((d,ii)<-df.dtypes.zipWithIndex)log.info(f"$ii%3d ${d._1}%30s ${d._2}%-15s")
    val maxNumRowsToShow=100
    log.info("number or rows in df="+df.count)
    log.info(showString(df,maxNumRowsToShow,80))
    val rc = df.repartition(desiredNumPartitions).write.option("compression","gzip").parquet(fidOut)
    prettyReNameMROutput(config,fs,lfs,false,fidOut)
...
//        msg="Launching yourStuffGoesHere pre-empted by this \"safty on\"\nComment this row to enable \"yourStuffGoesHere\" method.";println(msg);log.fatal(msg);System.exit(1);
        msg="SparkSession built, and connection to hiveMetaDataStore established. Launching yourStuffGoesHere";println(msg);log.fatal(msg);
